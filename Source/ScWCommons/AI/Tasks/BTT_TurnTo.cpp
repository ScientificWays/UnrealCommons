...
//~ Begin Task
uint16 UBTT_TurnTo::GetInstanceMemorySize() const // UBTNode
{
	return sizeof(FThisTaskMemory);
}

FString UBTT_TurnTo::GetStaticDescription() const // UBTNode
{
	return Common_GetStaticDescription(*this, bGetRotationFromTarget, BlackboardKey, Offset, ConstantSpeed);
}

void UBTT_TurnTo::DescribeRuntimeValues(const UBehaviorTreeComponent& InOwnerTree, uint8* InNodeMemory, EBTDescriptionVerbosity::Type InVerbosity, TArray<FString>& OutValues) const // UBTNode
{
...
EBTNodeResult::Type UBTT_TurnTo::ExecuteTask(UBehaviorTreeComponent& InOwnerTree, uint8* InNodeMemory) // UBTTaskNode
{
...
void UBTT_TurnTo::TickTask(UBehaviorTreeComponent& InOwnerTree, uint8* InNodeMemory, float InDeltaSeconds) // UBTTaskNode
{
...
void UBTT_TurnTo::OnTaskFinished(UBehaviorTreeComponent& InOwnerTree, uint8* InNodeMemory, EBTNodeResult::Type InTaskResult) // UBTTaskNode
{
	if (AATAAIController_Base* OwnerController = Cast<AATAAIController_Base>(InOwnerTree.GetAIOwner()))
	{
...
FString UBTT_TurnTo::Common_GetStaticDescription(const UBTNode& InNode, const bool bInGetRotationFromTarget, const FBlackboardKeySelector& InBlackboardKey, const FRotator& InOffset, const float InConstantSpeed)
{
...
void UBTT_TurnTo::Common_DescribeRuntimeValues(const UBTNode& InNode, const UBehaviorTreeComponent& InOwnerTree, uint8* InNodeMemory, EBTDescriptionVerbosity::Type InVerbosity, TArray<FString>& OutValues)
{
...
bool UBTT_TurnTo::Common_UpdateTargetRotation(UBTNode& InNode, UBehaviorTreeComponent& InOwnerTree, uint8* InNodeMemory, const FBlackboardKeySelector& InBlackboardKey, const FRotator& InOffset, const bool bInGetRotationFromTarget, const bool bInTurnToPawnViewLocation)
{
	AATAAIController_Base* OwnerController = Cast<AATAAIController_Base>(InOwnerTree.GetAIOwner());
...
EBTNodeResult::Type UBTT_TurnTo::Common_UpdateCurrentRotation(UBTNode& InNode, UBehaviorTreeComponent& InOwnerTree, uint8* InNodeMemory, float InDeltaSeconds, const float InConstantSpeed, const float InGoalTolerance, const bool bInFinishTaskWhenGoalReached, const bool bInDrawDebug)
{
