...
#include "AI/Decorators/BTD_CanActivateAbility.h"

UBTD_CanActivateAbility::UBTD_CanActivateAbility()
{
...
//~ Begin Decorator
/*uint16 UBTD_CanActivateAbility::GetInstanceMemorySize() const // UBTNode
{
	return sizeof(FThisTaskMemory);
}*/

FString UBTD_CanActivateAbility::GetStaticDescription() const // UBTNode
{
...
/*void UBTD_CanActivateAbility::OnBecomeRelevant(UBehaviorTreeComponent& InOwnerTree, uint8* InNodeMemory) // UBTAuxiliaryNode
{
...
bool UBTD_CanActivateAbility::CalculateRawConditionValue(UBehaviorTreeComponent& InOwnerTree, uint8* InNodeMemory) const // UBTDecorator
{
	if (AAIController* OwnerController = InOwnerTree.GetAIOwner())
	{
		if (UAbilitySystemComponent* OwnerASC = UAbilitySystemGlobals::GetAbilitySystemComponentFromActor(OwnerController->GetPawn()))
...
FGameplayAbilitySpec* UBTD_CanActivateAbility::TryGetAbilitySpec(const UBehaviorTreeComponent& InOwnerTree, const UAbilitySystemComponent* FromASC) const
{
