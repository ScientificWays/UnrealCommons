...
UBTS_GetPerceptionTarget::UBTS_GetPerceptionTarget()
{
...
//~ Begin Editor
#if WITH_EDITOR
void UBTS_GetPerceptionTarget::PostEditChangeProperty(FPropertyChangedEvent& InPropertyChangedEvent) // UObject
{
...
//~ Begin Service
void UBTS_GetPerceptionTarget::InitializeFromAsset(UBehaviorTree& InTreeAsset) // UBTNode
{
...
void UBTS_GetPerceptionTarget::OnBecomeRelevant(UBehaviorTreeComponent& InOwnerTree, uint8* InNodeMemory) // UBTAuxiliaryNode
{
	Super::OnBecomeRelevant(InOwnerTree, InNodeMemory);

	if (AAIController* OwnerController = InOwnerTree.GetAIOwner())
	{
		if (UATAAIPC_Base* BasePC = Cast<UATAAIPC_Base>(OwnerController->GetPerceptionComponent()))
		{
...
void UBTS_GetPerceptionTarget::OnCeaseRelevant(UBehaviorTreeComponent& InOwnerTree, uint8* InNodeMemory) // UBTAuxiliaryNode
{
...
void UBTS_GetPerceptionTarget::OnPerceptionUpdatedCallback(UBehaviorTreeComponent& InOwnerTree, uint8* InNodeMemory/*, const TArray<AActor*>& InUpdatedActors*/)
{
	if (AAIController* OwnerController = InOwnerTree.GetAIOwner())
	{
