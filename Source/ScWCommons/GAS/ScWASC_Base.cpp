...
#include "GAS/DataAssets/ComboMoveData.h"

UATAASC_Base::UATAASC_Base()
{
	AttributeSetClass = UATAAS_Base::StaticClass();

	bShouldDieOnZeroHealth = true;
}

//~ Begin Statics
UATAASC_Base* UATAASC_Base::TryGetBaseAtaASCFromActor(const AActor* InActor, bool bInTryFindComponentIfNoInterface)
{
	return Cast<UATAASC_Base>(UAbilitySystemGlobals::GetAbilitySystemComponentFromActor(InActor, bInTryFindComponentIfNoInterface));
}
//~ End Statics

//~ Begin Initialize
void UATAASC_Base::InitAbilityActorInfo(AActor* InOwnerActor, AActor* InAvatarActor) // UAbilitySystemComponent
{
	...
	InAvatarActor->OnTakePointDamage.AddDynamic(this, &UATAASC_Base::OnAvatarTakePointDamage);
	InAvatarActor->OnTakeRadialDamage.AddDynamic(this, &UATAASC_Base::OnAvatarTakeRadialDamage);
...
void UATAASC_Base::OnRegister() // UActorComponent
{
	Super::OnRegister();

	BaseAS = Cast<UATAAS_Base>(GetOrCreateAttributeSubobject(AttributeSetClass));
...
void UATAASC_Base::BeginPlay() // UActorComponent
{
	Super::BeginPlay();
...
//~ Begin Attributes
float UATAASC_Base::GetHealth() const
{
	if (BaseAS)
	{
...
float UATAASC_Base::GetMaxHealth() const
{
	if (BaseAS)
	{
...
bool UATAASC_Base::IsFullHealth() const
{
	if (BaseAS)
	{
...
void UATAASC_Base::SetHealth(float InHealth)
{
	if (TSubclassOf<UGameplayEffect> SetHealthEffectClass = UGASFunctionLibrary::GetSetHealthGameplayEffectClass(this))
	{
		TryApplyGameplayEffectByClass(SetHealthEffectClass, -1.0f, InHealth);
	}
	else
	{
		UE_LOG(LogAbilitySystemComponent, Warning, TEXT("UATAASC_Base::SetHealth() SetHealthEffectClass == nullptr! Can't set health."));
	}
}

void UATAASC_Base::AddHealth(float InHealth)
{
	if (TSubclassOf<UGameplayEffect> AddHealthEffectClass = UGASFunctionLibrary::GetAddHealthGameplayEffectClass(this))
	{
		TryApplyGameplayEffectByClass(AddHealthEffectClass, -1.0f, InHealth);
	}
	else
	{
		UE_LOG(LogAbilitySystemComponent, Warning, TEXT("UATAASC_Base::AddHealth() AddHealthEffectClass == nullptr! Can't add health."));
	}
}

void UATAASC_Base::OnHealthChanged(const FOnAttributeChangeData& InData)
{
...
void UATAASC_Base::OnMaxHealthChanged(const FOnAttributeChangeData& InData)
{
	OnMaxHealthChangedDelegate.Broadcast(InData.Attribute, InData.OldValue, InData.NewValue);
}

void UATAASC_Base::OnZeroHealth()
{
...
//~ Begin Abilities
void UATAASC_Base::GiveSpawnAbilities()
{
	ensure(SpawnAbilitiesSpecsHandles.IsEmpty());
	GiveAbilitiesWithLevels(SpawnAbilities, SpawnAbilitiesSpecsHandles);
}

void UATAASC_Base::GiveAbilitiesWithLevels(TMap<TSubclassOf<UGameplayAbility>, int32> InAbilitiesWithLevels, TArray<FGameplayAbilitySpecHandle>& OutHandleArray)
{
...
void UATAASC_Base::ClearAbilities(TArray<FGameplayAbilitySpecHandle>& InHandleArray, const bool bInResetArray)
{
...
//~ Begin Effects
FActiveGameplayEffectHandle UATAASC_Base::TryApplyGameplayEffectByClass(TSubclassOf<UGameplayEffect> InClass, float InDuration, float InMagnitude, float InLevel)
{
	if (InClass == nullptr)
	{
		UE_LOG(LogAbilitySystemComponent, Warning, TEXT("UATAASC_Base::TryApplyGameplayEffectByClass() InClass == nullptr! Can't apply GameplayEffect by class."));
...
int32 UATAASC_Base::TryRemoveGameplayEffectByClass(TSubclassOf<UGameplayEffect> InClass, int32 InStacksToRemove)
{
	if (InClass == nullptr)
	{
		UE_LOG(LogAbilitySystemComponent, Warning, TEXT("UATAASC_Base::TryRemoveGameplayEffectByClass() InClass == nullptr! Can't remove GameplayEffect by class."));
		return 0;
	}
...
void UATAASC_Base::ApplySpawnEffect()
{
	if (SpawnEffectClass)
	{
	...
	{
		GEngine->AddOnScreenDebugMessage(-1, 15.0f, FColor::Red, FString::Printf(TEXT("%s UATAASC_Base::ApplySpawnEffect() Invalid SpawnEffectClass! Possibly OnZeroHealth() will be called."), *GetName()));
	}
}
//~ End Effects

//~ Begin Damage
void UATAASC_Base::OnAvatarTakePointDamage(AActor* InDamagedActor, float InDamage, AController* InInstigator, FVector InHitLocation, UPrimitiveComponent* InHitComponent, FName InBoneName, FVector InHitDirection, const UDamageType* InDamageType, AActor* InDamageCauser)
{
...
void UATAASC_Base::OnAvatarTakeRadialDamage(AActor* InDamagedActor, float InDamage, const UDamageType* InDamageType, FVector InOrigin, const FHitResult& InHitResult, AController* InInstigator, AActor* InDamageCauser)
{
	HandleTryReceiveDamage(InDamage, { InHitResult, InDamageType, InDamageCauser, InInstigator });
}

bool UATAASC_Base::HandleTryReceiveDamage(float InDamage, const FReceiveDamageData& InData)
{
...
bool UATAASC_Base::ShouldIgnoreAnyAttackFrom(AController* InInstigator) const
{
...
bool UATAASC_Base::TryIgnoreDamage(float& InOutAdjustedDamage, const FReceiveDamageData& InData)
{
	check(IsOwnerActorAuthoritative());
...
bool UATAASC_Base::TryBlockDamage(float& InOutAdjustedDamage, const FReceiveDamageData& InData)
{
	check(IsOwnerActorAuthoritative());
...
bool UATAASC_Base::TryEvadeDamage(float& InOutAdjustedDamage, const FReceiveDamageData& InData)
{
	check(IsOwnerActorAuthoritative());

	const UATAAS_Base* AttributeSet = GetAttributeSet<UATAAS_Base>();
	check(AttributeSet);

	float EvasionChance = 0.0f;

	/*if (InData.DamageType->IsA(UATADT_Ranged::StaticClass()))
	{
		EvasionChance = AttributeSet->GetRangedDamageEvasionChance();
	}
	else if (InData.DamageType->IsA(UATADT_Melee::StaticClass()))
	{
...
bool UATAASC_Base::TryApplyDamage(float InDamage, const FReceiveDamageData& InData)
{
	check(IsOwnerActorAuthoritative());
	...
	const UATADT_Base* ATADamageType = Cast<UATADT_Base>(InData.DamageType);

	TSubclassOf<UGameplayEffect> ApplyDamageGameplayEffectClass = UGASFunctionLibrary::GetApplyDamageGameplayEffectClassForType(this, InData.DamageType);
	...
	{
		UE_LOG(LogAbilitySystemComponent, Log, TEXT("UATAASC_Base::TryApplyDamage() DamageEffectClass == nullptr! Can't apply damage."));
		return false;
	}
}
...
//~ Begin Combo
void UATAASC_Base::QueueComboMove(UComboMoveData* InComboMoveData)
{
	if (InComboMoveData)
	{
...
void UATAASC_Base::AcceptQueuedComboMove()
{
	if (QueuedComboMove)
	{
		UComboMoveData* AcceptedComboMove = QueuedComboMove;
...
void UATAASC_Base::DenyQueuedComboMove()
{
	if (QueuedComboMove)
	{
		UComboMoveData* DeniedComboMove = QueuedComboMove;
		QueuedComboMove = nullptr;
...
void UATAASC_Base::AddComboMove(UComboMoveData* InComboMoveData, bool InUpdateRelevantCombo, bool InResetIfNoRelevantCombo)
{
...
void UATAASC_Base::SetComboState(EComboState InState, bool InUpdateRelevantComboOnResetOrFail)
{
...
bool UATAASC_Base::UpdateRelevantComboFromCurrentMoves(bool InResetIfNoRelevantCombo)
{
